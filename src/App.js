...
const mapDriver = (r) => ({
  ...
  pen: r.pen || 0,
  avance: r.avance || 0,
  matricule:        r.matricule || r.driver_code || r.id,
  permisNum:        r.permisnum || r.permisNum || r.license_number || "",
  permisType:       r.permistype || r.permisType || r.license_type || "",
  permisDelivrance: r.permisdelivrance || r.permisDelivrance || r.license_issue_date || "",
  permisExpiration: r.permisexpiration || r.permisExpiration || r.license_expiry_date || "",
  pieceNum:         r.piecenum || r.pieceNum || r.id_card_number || "",
  pieceType:        r.piecetype || r.pieceType || r.id_card_type || "CNI",
  pieceDelivrance:  r.piecedelivrance || r.pieceDelivrance || r.id_card_issue_date || "",
  pieceExpiration:  r.pieceexpiration || r.pieceExpiration || r.id_card_expiry_date || "",
  typeContrat:      r.typecontrat || r.typeContrat || r.contract_type || "Salarie",
  contactUrgence:   r.contacturgence || r.contactUrgence || r.emergency_contact || "",
  contactUrgenceTel:r.contacturgencetel || r.contactUrgenceTel || r.emergency_phone || "",
  noteYango:        r.noteyango || r.noteYango || r.yango_score || 4.0,
  noteInterne:      r.noteinterne || r.noteInterne || r.internal_score || 80,
